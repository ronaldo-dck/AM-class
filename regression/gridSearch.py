...
dados = pd.read_csv("./datasets/dataset_normalized.csv")
...
# ##########################################################################################################
    ...
    for kernel in ("linear", "poly", "rbf", "sigmoid", "precomputed"):
        for C in [0.1, 1, 10]: #retirei o 200
            svm = SVR(kernel=kernel, C=C, probability=True, random_state=42)
    ...
    svm = SVR(kernel=best_kernel, C=best_C, probability=True, random_state=42)
    svm.fit(x_treino, y_treino)
...
##############################################################################
    ...
    for n_estimators in [10, 25, 50, 75, 100, 125, 150, 175, 200]:
